#ifndef XOROSHIRO128PLUSPLUS_HPP
#define XOROSHIRO128PLUSPLUS_HPP
...
struct xoroshiro128
{
   ...
   xoroshiro128() = default; 
	constexpr xoroshiro128( std::uint64_t s0, std::uint64_t s1 ): s{ s0, s1 }{}
	
	inline uint64_t rotl(const std::uint64_t x, int k) {
		return (x << k) | (x >> (64 - k));
	}
...
#endif //XOROSHIRO128PLUSPLUS_HPP